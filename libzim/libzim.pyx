...
cdef class _Creator:
    ...
    def config_nbworkers(self, int nbWorkers: pyint) -> Creator:
        """Configures the number of threads to use for internal workers.
    ...
    def add_illustration(self, int size: pyint, content: bytes):
        ...
        Args:
            size (int): The size of the illustration in bytes.
            content (bytes): The binary content of the PNG illustration.

        Raises:
            RuntimeError: If an illustration with the same size already exists.
        """
...
class IndexData:
    ...
    def get_title(self) -> str:
        ...
        Might be the same as `Item.get_title` or not.
...
class BaseWritingItem:
    ...
    def get_hints(self) -> Dict[Hint, pyint]:
        ...
        Returns:
            Hints to help the Creator decided how to handle this item.
        """
